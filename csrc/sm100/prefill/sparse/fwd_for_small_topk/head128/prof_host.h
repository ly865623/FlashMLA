...
enum ProfSlot {
    ...
    // FLASHMLA_PROF_MMA_LAT: single-MMA completion observed ON W8 ITSELF (the issuing warp),
    // via a suspending wait() on the completion barrier (incl. W8 wakeup; serializes the
    // sampled block only -> II / qk_occ / obs are NOT valid in an _MMA_LAT build).
    PROF_QK_SELF     = 8,  // W8: clock64 when W8's wait sees QK done -> L_QK = QK_SELF - ISSUE_P
    PROF_PV_SELF     = 9,  // W8: clock64 when W8's wait sees PV done -> L_PV = PV_SELF - ISSUE_O
    PROF_NUM_SLOTS   = 10
};
